...
import { Search, Star, TrendingUp, ShoppingCart, Share2, Copy, Check } from "lucide-react";
import { Button } from "@/components/ui/button";
import { Input } from "@/components/ui/input";
import { Dialog, DialogContent, DialogHeader, DialogTitle, DialogDescription, DialogFooter } from "@/components/ui/dialog";
import { dropshipProducts, DropshipProduct } from "@/data/dropshipProducts";
import { useToast } from "@/hooks/use-toast";

const categories = ["All", "Apparel", "Beauty", "Home & Kitchen", "Accessories", "Food & Beverage", "Electronics"];
...
const DropshipperProducts = () => {
  ...
  const filtered = dropshipProducts.filter((p) => {
  ...
  const handleCopyLink = (product: DropshipProduct) => {
    navigator.clipboard.writeText(`https://shop.prodpartner.com/p/${product.id}?ref=dropshipper123`);
  ...
  const handleCreateOrder = () => {
    ...
    toast({
      title: "Order Created!",
      description: `Order for ${orderQty}x ${selectedProduct.name}. You'll earn ৳${commission.toLocaleString()} commission.`,
    });
    setOrderOpen(false);
    setCustomerName("");
    setCustomerPhone("");
              ...
              <div className="p-5 space-y-3">
                ...
                <div className="flex items-center justify-between text-xs text-muted-foreground">
                  ...
                  <span className="flex items-center gap-1 text-primary font-medium">
                    ...
                    {((product.sellerProfit / product.dropshipPrice) * 100).toFixed(0)}% margin
                  </span>
                </div>
                ...
                <div className="flex gap-2 pt-1">
                  <Button
                    ...
                    onClick={() => { setSelectedProduct(product); setOrderOpen(true); }}
                  >
        ...
        <Dialog open={orderOpen} onOpenChange={setOrderOpen}>
          <DialogContent className="max-w-md">
            ...
            <DialogFooter>
              ...
              <Button onClick={handleCreateOrder} disabled={!customerName || !customerPhone || !customerAddress}>
                Confirm Order
              </Button>
            </DialogFooter>
          </DialogContent>
