    ...
    <section className="py-14 sm:py-20 md:py-28 px-4 sm:px-6">
      <div className="container max-w-4xl mx-auto">
        <div className="text-center mb-8 sm:mb-12">
          <p className="text-xs sm:text-sm font-medium text-primary mb-2 sm:mb-3">Unit Economics</p>
        ...
        <div className="border border-border rounded-xl sm:rounded-2xl bg-card overflow-hidden shadow-sm">
          {/* Example product header */}
          <div className="bg-muted/40 border-b border-border px-4 sm:px-6 py-3 sm:py-4">
            <div className="space-y-2.5 sm:space-y-3">
              <div className="flex items-center gap-2.5 sm:gap-3">
                ...
                <div className="min-w-0">
                  <p className="font-semibold text-xs sm:text-sm truncate">{EXAMPLE_PRODUCT.nameEn}</p>
                  <p className="text-[10px] sm:text-xs text-muted-foreground">{EXAMPLE_PRODUCT.category} • {EXAMPLE_PRODUCT.name}</p>
                </div>
              </div>
              <div className="grid grid-cols-3 gap-1.5 sm:gap-2 text-center">
                <div className="bg-background/50 rounded-lg py-1.5 sm:py-2 px-1">
                  <span className="text-muted-foreground block text-[9px] sm:text-[10px] leading-tight">প্রোডাকশন খরচ</span>
                  <span className="font-bold text-foreground text-xs sm:text-sm">৳{costPerUnit}</span>
                </div>
                <div className="bg-background/50 rounded-lg py-1.5 sm:py-2 px-1">
                  <span className="text-muted-foreground block text-[9px] sm:text-[10px] leading-tight">বিক্রয় মূল্য</span>
                  <span className="font-bold text-primary text-xs sm:text-sm">৳{retailPrice}</span>
                </div>
                <div className="bg-background/50 rounded-lg py-1.5 sm:py-2 px-1">
                  <span className="text-muted-foreground block text-[9px] sm:text-[10px] leading-tight">মার্জিন/ইউনিট</span>
                  <span className="font-bold text-primary text-xs sm:text-sm">৳{retailPrice - costPerUnit - logisticsPerUnit}</span>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="p-4 sm:p-6 md:p-8">
            ...
            <div className="mb-6 sm:mb-8">
              <div className="flex items-center justify-between mb-2.5 sm:mb-3">
                <span className="text-xs sm:text-sm text-muted-foreground">আপনি কতটি ইউনিট নিতে চান?</span>
                <div className="flex items-center gap-1 sm:gap-1.5 text-xs sm:text-sm font-bold">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 mb-5 sm:mb-6">
              <div className="rounded-xl border border-border bg-muted/30 p-4 sm:p-5">
                <div className="flex items-center gap-2 mb-1.5 sm:mb-2">
                  <div className="w-6 h-6 sm:w-7 sm:h-7 rounded-lg bg-muted flex items-center justify-center">
                    <Package className="w-3 h-3 sm:w-3.5 sm:h-3.5 text-muted-foreground" />
                  </div>
                  <span className="text-[10px] sm:text-xs text-muted-foreground">মোট বিনিয়োগ</span>
                </div>
              ...
              <div className="rounded-xl border border-primary/20 bg-primary/5 p-4 sm:p-5">
                <div className="flex items-center gap-2 mb-1.5 sm:mb-2">
                  ...
                  <span className="text-[10px] sm:text-xs text-muted-foreground">প্রত্যাশিত রিটার্ন</span>
                </div>
                <div className="text-xl sm:text-2xl md:text-3xl font-bold font-display text-primary">
              ...
              <div className={`rounded-xl p-4 sm:p-5 ${
                ...
                <div className="flex items-center gap-2 mb-1.5 sm:mb-2">
                  <div className={`w-6 h-6 sm:w-7 sm:h-7 rounded-lg flex items-center justify-center ${
                    ...
                    <BarChart3 className={`w-3 h-3 sm:w-3.5 sm:h-3.5 ${isPositive ? "text-primary" : "text-destructive"}`} />
                  </div>
                  <span className="text-[10px] sm:text-xs text-muted-foreground">নেট লাভ</span>
                </div>
                <div className={`text-xl sm:text-2xl md:text-3xl font-bold font-display ${
                ...
                <p className={`text-[10px] sm:text-xs mt-1 font-medium ${isPositive ? "text-primary/70" : "text-destructive/70"}`}>
            ...
            <button
              ...
              className="w-full flex items-center justify-center gap-2 text-[10px] sm:text-xs font-medium text-muted-foreground hover:text-foreground transition-colors py-2 mb-3 sm:mb-4"
            >
            ...
            {showBreakdown && (
              <div className="rounded-xl bg-muted/30 border border-border/50 p-3 sm:p-5 mb-5 sm:mb-6 space-y-2 sm:space-y-3 text-xs sm:text-sm">
                <h4 className="font-semibold text-[10px] sm:text-xs text-muted-foreground uppercase tracking-wider mb-2 sm:mb-3">খরচ ও আয়ের হিসাব</h4>
                ...
                <div className="space-y-1.5 sm:space-y-2">
                  ...
                  <div className="border-t border-border/40 pt-1.5 sm:pt-2" />
            ...
            {!isPositive && (
              <div className="rounded-lg bg-accent/50 border border-accent p-3 mb-5 sm:mb-6 text-[10px] sm:text-xs text-accent-foreground">
            ...
            <div className="text-center">
              <Link to="/signup" className="w-full sm:w-auto inline-block">
                <Button size="lg" className="w-full sm:w-auto h-12 sm:h-11 px-6 sm:px-8">
                  প্রোডাকশনে যোগ দিন
          ...
          <div className="border-t border-border px-4 sm:px-6 py-2.5 sm:py-3 bg-muted/30">
            <p className="text-[9px] sm:text-[11px] text-muted-foreground text-center leading-relaxed">
...
function Row({ label, value, bold, muted, highlight, negative }: {
  ...
  return (
    <div className="flex justify-between items-center gap-2">
      <span className={`${muted ? "text-muted-foreground" : "text-foreground"} text-[10px] sm:text-xs leading-tight`}>{label}</span>
      <span className={`font-mono text-[10px] sm:text-xs shrink-0 ${bold ? "font-bold" : "font-medium"} ${
